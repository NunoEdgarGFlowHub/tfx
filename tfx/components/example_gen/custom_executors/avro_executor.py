...
def _dict_to_example(instance: Dict[Text, Any]) -> tf.train.Example:
  """Decoded avro to tf example."""
  # Note that when convert to tf.Feature, Avro data might lose precision.
  feature = {}
  for key, value in instance.items():
    ...
    if value is None:
      ...
      if not value:
        ...
        raise RuntimeError(
            'Avro column type `list of {}` is not supported.'.format(
                type(value[0])))
    else:
      raise RuntimeError('Avro column type {} is not supported.'.format(
          type(value)))
...
def _AvroToExample(  # pylint: disable=invalid-name
  ...
  Args:
    ...
    input_dict: Input dict from input key to a list of Artifacts.
      - input-base: input dir that contains Avro data.
...
class Executor(base_example_gen_executor.BaseExampleGenExecutor):
  ...
  Data type conversion:
    ...
    Note that,
      ...
      Avro data might lose precision, e.g., int96.
  ...
  Example usage:
    ...
    example_gen = ExampleGen(executor=avro_executor.Executor,
